...
class ArithmeticCoderBase(object):
    ...
    def __init__(self, statesize):
        ...
        self.MAX_RANGE = 1 << self.STATE_SIZE # 2^32 = 4294967296
    ...
    def update(self,  cumul, symbol):
        ...
        total = cumul[-1].item()
        symlow = cumul[symbol].item() # 直接跳转到当前token id对应的数值上，作为新的下届
        symhigh = cumul[symbol+1].item() #相邻数值为上界，限制了第一次的范围
        #if symlow == symhigh:
